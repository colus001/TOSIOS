...
function generateTree(container: Container, iterations: number, args: DungeonArgs): TreeNode<Container> {
...
function splitContainer(
    ...
    if (direction === 'vertical') {
        ...
        if (leftWidthRatio < args.containerMinimumRatio || rightWidthRatio < args.containerMinimumRatio) {
        ...
        if (leftHeightRatio < args.containerMinimumRatio || rightHeightRatio < args.containerMinimumRatio) {
            return splitContainer(container, args, iterations - 1);
        }
    }
...
function generateCorridor(left: Container, right: Container, args: DungeonArgs): Corridor {
    // Create the corridor
...
function fillByType(tree: TreeNode<Container>, args: DungeonArgs, type: RoomType, count: number) {
...
// Tiles
//
function createTilesLayer(tree: TreeNode<Container>, args: DungeonArgs): TileMap {
...
// Props
//
function createPropsLayer(tree: TreeNode<Container>, tiles: TileMap, args: DungeonArgs): TileMap {
...
function carveTorches(tiles: TileMap, props: TileMap): TileMap {
    ...
    for (let y = 0; y < result.length; y++) {
        for (let x = 0; x < result[y].length; x++) {
            ...
            const leftCorner = maskToTileIdMap[TileDirection.North | TileDirection.West | TileDirection.NorthWest];
            const rightCorner = maskToTileIdMap[TileDirection.North | TileDirection.East | TileDirection.NorthEast];
...
// Monsters
//
function createMonstersLayer(tree: TreeNode<Container>, args: DungeonArgs): TileMap {
...
function computeBitMask(x: number, y: number, tiles: TileMap): number {
    ...
    if (mask & TileDirection.North && mask & TileDirection.West && tileDirectionCollides(x, y, 'north-west', tiles)) {
        mask |= TileDirection.NorthWest;
    }

    if (mask & TileDirection.North && mask & TileDirection.East && tileDirectionCollides(x, y, 'north-east', tiles)) {
        mask |= TileDirection.NorthEast;
    }

    if (mask & TileDirection.South && mask & TileDirection.West && tileDirectionCollides(x, y, 'south-west', tiles)) {
        mask |= TileDirection.SouthWest;
    }

    if (mask & TileDirection.South && mask & TileDirection.East && tileDirectionCollides(x, y, 'south-east', tiles)) {
...
function tileDirectionCollides(
    x: number,
    y: number,
    side: 'north' | 'west' | 'east' | 'south' | 'north-west' | 'north-east' | 'south-west' | 'south-east',
    tilemap: TileMap,
) {
    const isLeft = x === 0;
...
function findFittingTemplate(
    ...
    let result = sorted.find(
        (template) =>
            !usedIds.includes(template.id) && template.width <= container.width && template.height <= container.height,
    );

    if (!result) {
        result = sorted.find((template) => template.width <= container.width && template.height <= container.height);
    }

    return result;
}

function getTemplatesByType(templates: RoomTemplate[], type: RoomType): RoomTemplate[] {
...
function getRandomContainer(containers: Container[], usedIds: string[]): Container | null {
